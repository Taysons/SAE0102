...
public class StuckWin {
  ...
  StuckWinSTD.Result deplace(char couleur, String lcSource,
    ...
    if (!verifTaille(source) || !verifTaille(destination))
      return StuckWinSTD.Result.EXT_BOARD;
    // si la source dans State est vide
    if (state[source[0]][source[1]] == VIDE)
      return StuckWinSTD.Result.EMPTY_SRC;
    ...
    if (state[source[0]][source[1]] != couleur)
      return StuckWinSTD.Result.BAD_COLOR;
  ...
  public static void main(String[] args) {
    ...
    do {
      ...
      do {
        status = StuckWinSTD.Result.EXIT;
        ...
        status = jeu.deplace(curCouleur, src, dest, StuckWinSTD.ModeMvt.REAL);

        partie = jeu.finPartie(nextCouleur);

        System.out.println("status : " + status + " partie : " + partie);
      } while (status != StuckWinSTD.Result.OK && partie == 'N');
